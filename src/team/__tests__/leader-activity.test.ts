...
async function withTemp<T>(prefix: string, fn: (cwd: string) => Promise<T>): Promise<T> {
...
describe('leader runtime activity OMX parity adapter', () => {
  it('records team status activity under the OMC state root', async () => {
    await withTemp('omc-leader-activity-', async (cwd) => {
      ...
      const activity = JSON.parse(await readFile(join(cwd, '.omc', 'state', 'leader-runtime-activity.json'), 'utf-8')) as {
