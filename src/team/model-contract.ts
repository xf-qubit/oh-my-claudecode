...
function normalizeOptionalReasoning(
  reasoning?: TeamReasoningEffort | string | null,
...
export function normalizeTeamWorkerLaunchArgs(
  ...
  const reasoning =
    ...
    (normalizeOptionalReasoning(preferredReasoning)
      ...
      : null);
  const modelProvider =
    preferredModelProviderOverride ?? parsed.modelProviderOverride;
  if (modelProvider) normalized.push(CONFIG_FLAG, modelProvider);
  if (reasoning) normalized.push(CONFIG_FLAG, reasoning);

  const selectedModel =
    normalizeOptionalModel(preferredModel) ??
    normalizeOptionalModel(parsed.modelOverride);
  if (selectedModel) normalized.push(MODEL_FLAG, selectedModel);
  return normalized;
}

export function resolveTeamWorkerLaunchArgs(
...
export function buildLaunchArgs(
  ...
  const prepared = contractExtraFlags(
    agentType,
    config.extraFlags,
    config.model,
  );
